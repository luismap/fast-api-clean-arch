...
from features.votes.data.models.VotesModel import (
    VoteModel,
    VoteReadModel,
    VoteResponseModel,
)
    ...
    def create_vote(self, vote: VoteModel) -> VoteReadModel:
        with self.SessionLocal() as session:
            try:
                ...
                logging.info(e)
                raise HTTPException(
                    status.HTTP_409_CONFLICT, "conflict while adding vote"
                )
        ...
        if direction:
            ...
            if not vote_model:
                ...
                raise HTTPException(
                    ...
                    f"vote already exist for post_id {post_id}",
                )
        else:
            ...
            if not vote_model:
                raise HTTPException(
                    ...
                    f"post_id {post_id} not found for user {user_id}",
                )
            else:
